...
document.addEventListener('DOMContentLoaded', function() {
  ...
  musicItems.forEach(function(item, index) {
    ...
    var trackId = item.dataset.id; // Получаем уникальный ID трека

    if (!audioPlayer || !seekBar || !playButton || !trackId) return;

    audioPlayer.addEventListener("loadedmetadata", function() {
      seekBar.max = audioPlayer.duration;
      var savedTime = localStorage.getItem(`audioPosition_${trackId}`);
      if (savedTime) {
    ...
    playButton.addEventListener("click", function() {
      ...
      if (audioPlayer.paused) {
        ...
        audioPlayer.pause();
        localStorage.setItem(`audioPosition_${trackId}`, audioPlayer.currentTime);
        playButton.innerHTML = "&#9658;";
    ...
    audioPlayer.addEventListener("ended", function() {
      ...
      localStorage.removeItem(`audioPosition_${trackId}`);
    ...
    seekBar.addEventListener("change", function() {
      ...
      localStorage.setItem(`audioPosition_${trackId}`, seekBar.value);
    });
